...
```{r}
x_3_matrix = matrix(nrow=100, ncol=3)
currentRow = 1
for(i in x_3_ord){
  
  if(i == "none"){
    
    x_3_matrix[currentRow,]<-c(0,0,0)
    currentRow = currentRow + 1
    
  } else if (i == "infraction"){
    x_3_matrix[currentRow, ] <-  c(1,0,0)
    
    currentRow = currentRow + 1
  } else if (i == "misdimeanor"){
    x_3_matrix[currentRow, ] <-  c(0,1,0)
    currentRow = currentRow + 1
  } else if (i == "felony"){
    x_3_matrix[currentRow, ] <-  c(0,0,1)
...
if the sum of a row =  1 then that person has some sort of criminal record
...
```{r}
number_of_people_with_records = sum(rowSums(x_3_matrix))
print(paste("The Number of people with records is:", number_of_people_with_records))
```
...
fifth_column_binomial = rbinom(n=20,size=length(fake_first_names), p=0.12 )
sixth_column_binary = sample(
   c(0,1),
   size=length(fake_first_names),
   replace = TRUE,
   prob=c(.76,.24)
 )

fake_first_names_matrix = matrix(,nrow =length(fake_first_names), ncol=6, dimnames = list(fake_first_names), c(1:6) )
for (r in 1:nrow(fake_first_names_matrix)){
    fake_first_names_matrix[r,1] = rnorm(1, mean=17, sd= 38)
    fake_first_names_matrix[r,2] = runif(1, min=-10, max=10)
    fake_first_names_matrix[r,3] = rpois(1, 6)
    fake_first_names_matrix[r,4] = rexp(1, 9)
    fake_first_names_matrix[r,5] = sample(fifth_column_binomial, 1)
    fake_first_names_matrix[r,6] = sixth_column_binary[r]
    
}
fake_first_names_matrix

sum(sixth_column_binary)
...
```{r}
dataF = data.frame(fake_first_names_matrix)
```
...
percent_domestic = sum(dataF$X6 == 0)/ length(dataF$X6)
percent_foreign = sum(dataF$X6 == 1)/ length(dataF$X6)
```
...
n=50
result = rep(c(0,1,2), round((n*n)*c(.50,.25,.25)))
R = matrix(data=sample(result), nrow = n, ncol = n)
...
```{r}
holes = rep(c(0,1), round((n*n)*c(0.7,0.3)))
holes_matrix = matrix(data=sample(holes), nrow=n, ncol=n)

for (r in 1:nrow(R)){
    for (c in 1:ncol(R)){
        if(holes_matrix[r,c] == 1){
          R[r,c] = NA
        }
    }
}
...
```{r}
test <- R[order(rowSums(R, na.rm = TRUE), decreasing = TRUE)]
new_matrix = matrix(data=NA,ncol=n, nrow=n)

Rsums = rowSums(R, na.rm = TRUE)
for(i in 1:nrow(R)){
  new_matrix[i,] <- R[which.min(Rsums),]
  Rsums[which.min(Rsums)] = NA
}

rowSums(new_matrix, na.rm = TRUE)
```
...
```{r}
data <- data.frame(A1=round(rnorm(25,5,4)),A2=round(rnorm(25,5,4)),A3=round(rnorm(25,5,4)),
A4=round(rnorm(25,5,4)),A5=round(rnorm(25,5,4)),A6=round(rnorm(25,5,4)))

data_deviation = apply(data[,1:6],2,sd)
...
```{r}
data_count_entries = apply(data[,1:6],2,function(n){length(which(n==1)) + length(which(n==2))})
data
```
...
```{r}
data_split = split(data, rep(1:nrow(data)))
```
...
```{r}
lapply(row(new_matrix),function(x){
  c(
  min = min(x),
  max = max(x),
  pct_missing = (is.na(x)/length(x)) * 100,
  first_NA = match(NA, x)
  )
})
```
...
```{r}
qnorm(p=0.05, mean=v_average, sd=sd(v))
```
