...
X = matrix(rnorm(100), nrow = 25, ncol=25)
flip_matrix(X)
```
...
my_list=list()
LETTERS
my_list[["A"]] = array(data=1:4, dim=c(2,2))
...
lorem_split = strsplit(lorem, split=". ", fixed=TRUE)
lorem_sample = sample(lorem_split[[1]])
```
...
myList = list("M" = list(), "F" = list())

myList$M$Boomer = M_Boomer
myList$M$GenX = M_GenX
myList$M$Millennial = M_Millennial
myList$F$Boomer = F_Boomer
myList$F$Gen_X = F_GenX
myList$F$Millennial = F_Millennial
...
X$name = rep(unlist(myList), times = 14 * 3)
...
```{r}
#' One Neareset Neighbor Classifier
#'
...
```{r}
test_input = matrix( cbind(c(1, 1, 2, 3, 3, 4), c(1, 2, 1, 3, 4, 3)) )
test_data = matrix( c(1, 3, 4, 2, 1, 2) )
binary_data = factor(c(0, 1, 0, 1, 0, 1))
nn_algorithm_predict(test_input, binary_data, test_data)

test_input = matrix( cbind(c(1, 2, 1, 3, 1, 4), c(2, 4, 1, 3, 2, 1)) )
test_data = matrix( c(1, 2, 3, 4, 1, 2) )
binary_data = factor(c(1, 1, 0, 1, 0, 0))
nn_algorithm_predict(test_input, binary_data, test_data)
```
...
#' One Neareset Neighbor Classifier
#'
#' Classify an observation based on the label of the colosest observation in the set of training observations
#'
#' @param Xinput      A matrix of features for training data observations
#' @param y_binary    The vector of training data labels
#' @param Xtest       A test observation as a row vector
...
nn_algorithm_predict = function(Xinput, y_binary, Xtest, d=function(v1,v2){sum((v1-v2)^2)}){
  ...
  distances = array(NA, n)
  for(i in 1:n){
    distances[i] = d((Xinput[i,]-Xtest)^2)
...
TO-DO: gives an overview of the iris data including variable means and standard deviation
...
for (i in 1:n){
  y_length <- as.numeric(iris$Sepal.Length > iris$Sepal.Length[i])
  numErrors[i] <- sum(y_length != y)
}
...
```{r}
sum( as.numeric(iris$Sepal.Length > threshold) != y)
```
...
Yes since in the 75th percentile of the data of versicolors is greater than the threshold 5.4 and less than 
the length of setosas
...
This graph above show the binary response of the first_feature and the second_feature
...
The first number represents the intercept
The second number is the slope
The algebra equation resulting from the out put is y = 4x + 7
...
It is not the most optimal way to sperate the data in the graph
